...
OPS = [
    BlasOp('gemv', (float, double),
           [trans('transA'), size('N'), size('M'), scalar('alpha'),
...
% for op in ops:
#define ${op.name.upper()}(dtype, typec, TYPEC)				\\
  static int typec ## ${op.name}(${op.format_arguments('dtype')}) {	\\
    FETCH_CONTEXT(${op.array_args()[0].name});				\\
    FUNC_DECLS;								\\
    PREP_ORDER_${op.name.upper()};		                        \\
									\\
    HANDLE_ORDER_${op.name.upper()};	                                \\
    FUNC_INIT;								\\
									\\
...
% endfor
									\\
    PRE_CALL __GLUE(PREFIX(typec, TYPEC), ${op.name})(INIT_ARGS ${op.format_call_args()} TRAIL_ARGS); \\
    POST_CALL;								\\
									\\
% for a in op.array_args():
    ARRAY_FINI(${a.name});						\\
...
try:
    generic = GENERIC_TMPL.render(ops=OPS)
