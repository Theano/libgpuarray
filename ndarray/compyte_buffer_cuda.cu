                     ...
                     void * src, size_t src_offset, size_t sz)
{
    dst += dst_offset;
    src += src_offset;
    cudaError_t err = cudaMemcpy(dst, src, sz, cudaMemcpyDeviceToDevice);
    CNDA_THREAD_SYNC;
    if (cudaSuccess != err) {
        return -1;
    }
...
static compyte_buffer_ops _cuda_ops = {cuda_alloc, cuda_free, cuda_move, cuda_read, cuda_write, cuda_memset, NULL, NULL};
